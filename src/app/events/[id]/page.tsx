...
export default function EventPage() {
  ...
  const [showAbhedya, setShowAbhedya] = useState(false);
  ...
  return (
    <>
          ...
          <div style={{ textAlign: "center", paddingTop: "2rem", borderTop: "1px solid rgba(180,124,60,0.12)" }}>
            ...
            <button
              ...
              onClick={() => {
                ...
                if (event.title?.toLowerCase() === "abhedya") {
                  setShowAbhedya(true);
                  return;
                }
                setShowModal(true);
      ...
      {showModal && (
       ...
       {showAbhedya && (
        <div
          onClick={() => setShowAbhedya(false)}
          style={{ position: "fixed", inset: 0, zIndex: 100, background: "rgba(4,3,2,0.9)", display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }}
        >
          <div
            onClick={e => e.stopPropagation()}
            style={{ position: "relative", width: "100%", maxWidth: 380, background: "#0e0b06", border: "1px solid rgba(180,124,60,0.25)", borderRadius: 8, padding: "2rem 1.75rem", textAlign: "center" }}
          >
            <button
              onClick={() => setShowAbhedya(false)}
              style={{ position: "absolute", top: 12, right: 16, background: "none", border: "none", color: "rgba(180,124,60,0.5)", cursor: "pointer", fontSize: "1.2rem", lineHeight: 1 }}
            >✕</button>

            <div style={{ fontSize: "2rem", marginBottom: "0.75rem" }}>𓂀</div>

            <h2 style={{ fontFamily: "'Barlow Condensed',sans-serif", fontWeight: 700, fontSize: "1.4rem", letterSpacing: "0.08em", textTransform: "uppercase", color: "#f5ead8", margin: "0 0 0.75rem" }}>
              Abhedya
            </h2>

            <p style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: "italic", fontSize: "1rem", color: "rgba(200,146,78,0.65)", lineHeight: 1.6, marginBottom: "1.5rem" }}>
              Website coming soon. Join our community to stay updated on registration and announcements.
            </p>
